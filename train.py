...
def get_ds(config, fraction=1.0):
    # It only has the train split, so we divide it overselves
    ds_raw = load_dataset(f"{config['datasource']}",
                          f"{config['lang_src']}-{config['lang_tgt']}", split='train')
    ds_raw = ds_raw.select(range(int(len(ds_raw) * fraction)))  # use a fraction of the dataset
    ...
    for item in tqdm(ds_raw, desc=f"Encoding dataset."):
...
def train_model(config):
    ...
    device = "cuda" if torch.cuda.is_available() else "mps" \
        if torch.has_mps or torch.backends.mps.is_available() else "cpu"
    ...
    if device == 'cuda':
        ...
        print(" On a Mac machine, run: pip3 install --pre torch torchvision torchaudio torchtext "
    ...
    # ignore_index: ignore the padding tokens to contributed to the loss.
    # label_smoothing: smooth the highest probability to increase the accuracy of the model.
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # decoder_output: (B, seq_len, d_model)
...
if __name__ == "__main__":
    print('Train a transformer for translation.')
